...
> **ACTION REQUIRED**: Replace these placeholder values with your project's brand colors.
> The primary color must also be set in:
>   - `.env.local` → `NEXT_PUBLIC_PRIMARY_COLOR`
>   - `web/src/app/globals.css` → `--color-primary`
>   - `web/src/app/brand-overrides.css` → all hex values

| Role       | Value       | Usage                                |
| ---------- | ----------- | ------------------------------------ |
| `primary`  | `<<YOUR BRAND PRIMARY HEX e.g. #E8A020>>` | Buttons, CTAs, highlights |
| `primary-dark` | `<<DARKER SHADE e.g. #C8841A>>` | Button hover, gradient end |
| `primary-light` | `<<LIGHTER SHADE e.g. #F5C842>>` | Gradient highlight |
| `accent`   | `#a855f7` | Gradient endpoints (change if needed) |
...
| Name              | Value                                           | Usage            |
| ----------------- | ----------------------------------------------- | ---------------- |
| `hero-gradient`   | `from-slate-950 via-slate-900 to-brand-950`     | Page backgrounds |
| `brand-gradient`  | `from-primary to-primary-dark`                  | Buttons, headers |
| `card-glow`       | `radial-gradient(rgba(PRIMARY_RGB, 0.1), transparent)` | Card hover  |
